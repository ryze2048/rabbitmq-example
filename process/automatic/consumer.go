...
func (a *Automatic) Consumer(ctx context.Context) {
	global.ZAPLOG.Info(fmt.Sprintf("start consumer consumer --> %v", time.Now().Format("2006-01-02 15:04:05")))
	...
	for {
		...
		case data, ok := <-message:
			...
			global.ZAPLOG.Info(fmt.Sprintf("body info --> %s", string(data.Body)))
		case <-ctx.Done():
